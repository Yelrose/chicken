...
if __name__ == "__main__":
    ...
    for i in xrange(sz):
        ...
        if course_id not in mark:
            mark[course_id] = {}
        if object_csv["category"][i] != "video":continue
        video_id = object_csv["module_id"][i]
        mark[course_id][video_id] = len(mark[course_id])
    ...
    for i in xrange(sz):
        ...
        if log_train["event"][i] != "video": continue
        video_id = log_train["object"][i]
        if video_id not in mark[course_id]:continue
        print enrollment_id,video_id
        dic[enrollment_id][mark[course_id][video_id]] = 1
    ...
    for i in xrange(sz):
        ...
        if enrollment_id not in dic:
            dic[enrollment_id] = np.zeros(len(mark[course_id]),dtype="int")
        if log_test["event"][i] != "video": continue
        video_id = log_test["object"][i]
        if video_id not in mark[course_id]:continue
        print enrollment_id,video_id
        dic[enrollment_id][mark[course_id][video_id]] = 1


    writer = csv.writer(file('video_vector.csv','wb'))
    writer.writerow(["enrollment_id","video_vector"])
